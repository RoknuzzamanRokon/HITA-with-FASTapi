...
class ExportEngine:
    ...
    def export_mappings_sync(
        ...
        try:
            ...
            # Generate file based on format
            if format == ExportFormat.CSV:
                headers = ['ittid', 'provider_name', 'provider_id', 'system_type', 
                          'vervotech_id', 'giata_code', 'created_at', 'updated_at']
    ...
    def export_supplier_summary_sync(
        ...
        try:
            ...
            # Generate file based on format
            if format == ExportFormat.CSV:
                headers = ['provider_name', 'total_hotels', 'total_mappings', 
    ...
    def export_hotels_async(
        ...
        """
        logger.info(f"Starting asynchronous hotel export for user {user.id} in {format.value} format")
    ...
    def process_async_hotel_export(
        ...
        try:
            ...
            def progress_tracking_generator():
                ...
                for batch in self.stream_query_results(query, self.batch_size):
                    ...
                    if total_records > 0:
                        ...
                        if progress >= last_progress_update + 10:
                            ...
                            logger.debug(f"Export job {job_id} progress: {progress}%")
    ...
    def export_mappings_async(
        ...
        Returns:
            ExportJob object with job details
        """
        logger.info(f"Starting asynchronous mapping export for user {user.id} in {format.value} format")
    ...
    def export_supplier_summary_async(
        ...
        Returns:
            ExportJob object with job details
        """
        logger.info(f"Starting asynchronous supplier summary export for user {user.id} in {format.value} format")
