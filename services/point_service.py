...
class PointService:
    ...
    def get_user_point_summary(self, user_id: str) -> Optional[PointSummary]:
        ...
        try:
            user_points = (
                self.db.query(UserPoint).filter(UserPoint.user_id == user_id).first()
            )
    ...
    def get_user_transaction_summary(
        ...
        try:
            ...
            logger.error(
                f"Error getting transaction summary for user {user_id}: {str(e)}"
            )
            raise
    ...
    def allocate_points(
        ...
        try:
            ...
            user_points = (
                self.db.query(UserPoint)
                .filter(UserPoint.user_id == receiver_id)
                .first()
            )
            ...
            # Create notification for significant point allocations (>= 100 points)
            if points >= 100:
                try:
                    notification_service = NotificationService(self.db)
                    notification_service.notify_point_transaction(
                        user_id=receiver_id,
                    ...
                    logger.error(
                        f"Failed to create notification for point allocation: {str(e)}"
                    )
    ...
    def deduct_points(
        self, user_id: str, points: int, reason: str = "deduction"
    ) -> bool:
        """
        ...
        try:
            ...
            # Create notification for significant point deductions (>= 100 points)
            if points >= 100:
                try:
                    notification_service = NotificationService(self.db)
                    notification_service.notify_point_transaction(
                    ...
                    logger.error(
                        f"Failed to create notification for point deduction: {str(e)}"
                    )
    ...
    def transfer_points(self, giver_id: str, receiver_id: str, points: int) -> bool:
        ...
        try:
            logger.info(
                f"Transferring {points} points from {giver_id} to {receiver_id}"
            )

            # Get giver points
            giver_points = (
                self.db.query(UserPoint).filter(UserPoint.user_id == giver_id).first()
            )
            ...
            logger.info(
                f"Successfully transferred {points} points from {giver_id} to {receiver_id}"
            )

            # Create notification for significant point transfers (>= 100 points)
            if points >= 100:
                try:
                    notification_service = NotificationService(self.db)
                    # Get giver username for notification
                    giver = self.db.query(User).filter(User.id == giver_id).first()
                    ...
                    notification_service.notify_point_transaction(
                        ...
                        points=points,
                        from_user=giver_username,
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to create notification for point transfer: {str(e)}"
                    )
    ...
    def calculate_user_activity_status(self, user_id: str, days: int = 7) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error calculating activity status for user {user_id}: {str(e)}"
            )
            return "Unknown"
    ...
    def get_point_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            users_with_points = (
                self.db.query(UserPoint).filter(UserPoint.current_points > 0).count()
            )
