"""
Hotel Mapping Module - Direct Details Retrieval
===============================================

This module provides hotel mapping functionality that gets data directly from 
/hotel/details endpoint without requiring the /hotel/pushhotel step.

Key Features:
- Direct hotel details retrieval
...
class HotelMapper:
    """Enhanced hotel mapping class - gets data directly from /hotel/details without push"""
...
def main():
    """Main function with configurable parameters - Direct details retrieval"""
    # Configuration
    supplier_name = "agoda"
    hotel_id = "297844"
    
    print(f"\n{'='*60}")
    print("HOTEL MAPPING - DIRECT DETAILS RETRIEVAL")
    print(f"{'='*60}")
    print(f"Supplier: {supplier_name}")
    print(f"Hotel ID: {hotel_id}")
    print(f"Method: Direct /hotel/details (no push required)")
    print(f"{'='*60}")
...
def test_direct_details_retrieval():
    """Test the direct details retrieval functionality"""
    test_cases = [
        {"supplier": "agoda", "hotel_id": "297844"},
        {"supplier": "hotelbeds", "hotel_id": "12345"},  # Add more test cases as needed
    ]
    
    mapper = HotelMapper()
    
    print(f"\n{'='*70}")
    print("TESTING DIRECT HOTEL DETAILS RETRIEVAL")
    print(f"{'='*70}")
    
    for i, case in enumerate(test_cases, 1):
        print(f"\nTest Case {i}: {case['supplier']} - {case['hotel_id']}")
        print("-" * 50)
        
        # Test just the details retrieval
        details = mapper.get_hotel_details(case['supplier'], case['hotel_id'])
        if details:
            print("✅ Hotel details retrieved successfully")
            print(f"Hotel Name: {details.get('name', 'N/A')}")
            
            # Extract location info
            city, country = extract_city_country_from_api(details)
        ...
        if result:
            ...
            match_info = result['find_hotel']['matching_info']
            print(f"Confidence Score: {match_info['confidence_score']}")
        else:
            print("❌ Mapping failed or no match found")
...
if __name__ == "__main__":
    # Run main mapping example
    main()
    
    # Uncomment to run direct details retrieval tests
    # test_direct_details_retrieval()