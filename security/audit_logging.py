...
class AuditLogger:
    ...
    def log_activity(
        ...
        success: bool = True,
    ) -> Optional[models.UserActivityLog]:
        """
        Log user activity
        ...
        Returns:
            Created audit log entry or None if logging was skipped
        """
        ...
        if request and hasattr(request, "url"):
            try:
                ...
                # If endpoint is in exclude list, skip logging completely
                if api_logging_config.should_exclude_endpoint(endpoint_path):
                    print(f"⏭️ AUDIT SKIP: {endpoint_path} is in exclude list")
                    return None

                # If endpoint is not in count list and it's an API_ACCESS activity, skip it
                if (
                    activity_type == ActivityType.API_ACCESS
                    and not api_logging_config.should_count_endpoint(endpoint_path)
                ):
                    print(f"⏭️ AUDIT SKIP: {endpoint_path} not in count list")
                    return None

            except Exception as e:
                # If config check fails, continue with logging (fail-safe)
                print(f"⚠️ Config check failed, continuing with logging: {e}")
    ...
    def log_security_event(
        ...
        security_details.update(
            {
                "security_event": True,
                "requires_investigation": security_level
    ...
    def get_user_activity_history(
        ...
        query = self.db.query(models.UserActivityLog).filter(
            ...
            models.UserActivityLog.created_at
    ...
    def get_security_events(
        ...
        if security_level:
            query = query.filter(
                models.UserActivityLog.details.contains(
...
class SessionManager:
    ...
    def create_session(
        self, user_id: str, session_id: str, request: Request, expires_at: datetime
    ) -> models.UserSession:
    ...
    def end_session(
        self, session_id: str, user_id: str, request: Optional[Request] = None
    ):
        """End a user session"""
        session = (
            self.db.query(models.UserSession)
