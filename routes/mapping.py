...
def check_ip_whitelist(user_id: str, request: Request, db: Session) -> bool:
    ...
    try:
        ...
        whitelist_entry = (
            ...
            .filter(
                models.UserIPWhitelist.user_id == user_id,
                models.UserIPWhitelist.ip_address == client_ip,
                models.UserIPWhitelist.is_active == True,
            )
            .first()
        )
...
def add_rate_type(
    ...
    try:
        ...
        provider_mapping = (
            ...
            .filter_by(id=provider_data.provider_mapping_id)
            .first()
        )
...
def get_basic_mapping_with_info(
    supplier_name: List[str] = Query(
        ..., description="List of provider names to filter by"
    ),
    country_iso: List[str] = Query(
                    ...
                    results.append(
                        {
                            ...
                            "longitude": (
                                float(hotel.longitude) if hotel.longitude else None
                            ),
                            "latitude": (
                                float(hotel.latitude) if hotel.latitude else None
                            ),
                            ...
                            "address": " ".join(
                                filter(None, [hotel.address_line1, hotel.address_line2])
                            ),
...
def get_mapping_with_ittid(
    ...
    try:
        ...
        if not request.ittid or not request.ittid.strip():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="ITTID cannot be empty"
            )

        # Check if hotel exists
        hotel_exists = (
            db.query(models.Hotel).filter(models.Hotel.ittid == request.ittid).first()
        )
        ...
        all_mappings = (
            db.query(models.ProviderMapping)
            .filter(models.ProviderMapping.ittid == request.ittid)
            .all()
        )
        ...
        if current_user.role not in [
            ...
            temp_deactivated = {
                perm[0].replace("TEMP_DEACTIVATED_", "") for perm in user_permissions
            }
