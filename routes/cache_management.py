...
router = APIRouter(prefix="/v1.0/cache", tags=["Cache Management"])
...
async def cache_health_check(
    ...
    """
    Basic Cache Health Check (Admin Only)
        ...
        if cache.is_available:
            try:
                ...
                try:
                    key_counts = {
                        "user_stats_keys": len(
                            cache.redis_client.keys(f"{CacheKeys.USER_STATS}*")
                        ),
                        "user_list_keys": len(
                            cache.redis_client.keys(f"{CacheKeys.USER_LIST}*")
                        ),
                        "user_details_keys": len(
                            cache.redis_client.keys(f"{CacheKeys.USER_DETAILS}*")
                        ),
                        "dashboard_stats_keys": len(
                            cache.redis_client.keys(f"{CacheKeys.DASHBOARD_STATS}*")
                        ),
                    ...
                    key_counts = {
                        "error": f"Could not retrieve key counts: {str(key_error)}"
                    }
                ...
                memory_info = {
                    ...
                    "memory_fragmentation_ratio": redis_info.get(
                ...
                if total_requests > 0:
                    performance_info["hit_rate"] = round(
                        (keyspace_hits / total_requests) * 100, 2
                    )
                    performance_info["miss_rate"] = round(
                        (keyspace_misses / total_requests) * 100, 2
                    )
...
async def clear_all_caches(
    ...
    try:
        ...
        logger.info(
            f"All caches cleared by user: {current_user.id} ({current_user.username if hasattr(current_user, 'username') else 'unknown'})"
        )
        ...
        logger.error(
            f"Cache clear operation failed: {e} - User: {getattr(current_user, 'id', 'unknown')}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def clear_user_cache(
    ...
    try:
        ...
        # Log successful operation
        logger.info(
            f"User cache cleared for {user_id} by admin: {current_user.id} ({getattr(current_user, 'username', 'unknown')})"
        )
        ...
        logger.error(
            f"User cache clear failed for user {user_id}: {e} - Admin: {getattr(current_user, 'id', 'unknown')}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def warm_caches(
    ...
    try:
        ...
        # Log successful operation
        logger.info(
            f"Cache warming initiated by admin: {current_user.id} ({getattr(current_user, 'username', 'unknown')})"
        )
        ...
        logger.error(
            f"Cache warming failed: {e} - Admin: {getattr(current_user, 'id', 'unknown')}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def cache_statistics(
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    performance_metrics = {
                        ...
                        "total_requests": total_requests,
                        "hit_rate_percentage": (
                            round((keyspace_hits / total_requests) * 100, 2)
                            if total_requests > 0
                            else 0
                        ),
                    ...
                    performance_metrics = {
                        "error": f"Performance metrics unavailable: {str(perf_error)}"
                    }
                ...
                performance_metrics = {
                    "error": "Performance metrics unavailable due to key count error"
                }
        else:
        ...
        if isinstance(key_counts, dict) and "error" not in key_counts:
            ...
            if total_keys > 0:
                usage_patterns = {
                    "user_stats_percentage": round(
                        (key_counts.get("user_stats_keys", 0) / total_keys) * 100, 2
                    ),
                    "user_list_percentage": round(
                        (key_counts.get("user_list_keys", 0) / total_keys) * 100, 2
                    ),
                    "user_details_percentage": round(
                        (key_counts.get("user_details_keys", 0) / total_keys) * 100, 2
                    ),
                    "dashboard_stats_percentage": round(
                        (key_counts.get("dashboard_stats_keys", 0) / total_keys) * 100,
                        2,
                    ),
                    "most_used_cache_type": (
                        max(key_counts, key=key_counts.get) if key_counts else "unknown"
                    ),
                }
        ...
        if isinstance(performance_metrics, dict) and "error" not in performance_metrics:
            ...
            if hit_rate > 95:
                recommendations.append(
                    "Excellent cache performance - consider current configuration as baseline"
                )

            total_keys = (
                key_counts.get("total_keys", 0) if isinstance(key_counts, dict) else 0
            )
            if total_keys > 10000:
                recommendations.append(
                    "High key count - monitor memory usage and consider cleanup"
                )

        # Log statistics access
        logger.info(
            f"Cache statistics accessed by admin: {current_user.id} ({getattr(current_user, 'username', 'unknown')})"
        )
        ...
        logger.error(
            f"Cache statistics retrieval failed: {e} - Admin: {getattr(current_user, 'id', 'unknown')}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        ...
        if not cache.is_available:
            return {
                ...
                "recommendations": [
                    "Check cache system configuration and connectivity"
                ],
            }
        ...
        performance_metrics["set_duration_ms"] = (
            set_end - set_start
        ).total_seconds() * 1000
        ...
        performance_metrics["get_duration_ms"] = (
            get_end - get_start
        ).total_seconds() * 1000
        ...
        performance_metrics["exists_duration_ms"] = (
            exists_end - exists_start
        ).total_seconds() * 1000
        ...
        performance_metrics["delete_duration_ms"] = (
        ...
        avg_operation_time = sum(performance_metrics.values()) / len(
            performance_metrics
        )
        performance_assessment = (
            "excellent"
            if avg_operation_time < 10
            else "good" if avg_operation_time < 50 else "needs_attention"
        )
        ...
        if not overall_success:
            recommendations.append(
                "Cache system has operational issues - investigate failed operations"
            )
        if avg_operation_time > 100:
            recommendations.append(
                "Cache operations are slow - check system performance and network connectivity"
            )
        if not test_results["data_integrity"]:
            recommendations.append(
                "Data integrity issues detected - verify cache serialization/deserialization"
            )
        ...
        # Log test execution
        logger.info(
            f"Cache operation test completed - Success: {overall_success}, Avg time: {avg_operation_time:.2f}ms"
        )

        return {
            "success": overall_success,
            "timestamp": test_start_time.isoformat(),
            "test_duration_ms": (datetime.utcnow() - test_start_time).total_seconds()
            * 1000,
            "test_results": test_results,
            "performance_metrics": performance_metrics,
            ...
            "cache_health": {
                ...
                "operation_reliability": sum(critical_operations)
                / len(critical_operations)
                * 100,
            },
            "recommendations": recommendations,
            "test_metadata": {
