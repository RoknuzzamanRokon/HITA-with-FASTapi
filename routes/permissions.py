...
def deactivate_suppliers_temporarily(
    ...
    try:
        ...
        not_found_suppliers = [
            name for name in supplier_names if name not in user_accessible_suppliers
        ]
        ...
        for supplier_name in supplier_names:
            existing = (
                ...
                .filter(
                    ...
                    == f"TEMP_DEACTIVATED_{supplier_name}",
                )
                .first()
            )
...
def activate_suppliers(
    ...
    try:
        ...
        temp_deactivated_names = [
            f"TEMP_DEACTIVATED_{name}" for name in request.supplier_name
        ]
...
def get_user_ip_whitelist(
    ...
    try:
        ...
        ip_entries = (
            db.query(UserIPWhitelist)
            .filter(
                UserIPWhitelist.user_id == user_id, UserIPWhitelist.is_active == True
            )
            .all()
        )
        ...
        return {
            ...
            "ip_whitelist": {"total_entries": len(ip_list), "entries": ip_list},
            "managed_by": {"id": current_user.id, "username": current_user.username},
        }
