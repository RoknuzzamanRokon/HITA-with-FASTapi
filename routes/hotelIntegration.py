...
def create_hotel_with_details(
    ...
    try:
        ...
        if not hotel:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Hotel data is required"
            )
        ...
        try:
            ...
            hotel_data = hotel.dict(
                exclude={"locations", "provider_mappings", "contacts", "chains"}
            )
                ...
                for location_data in hotel.locations:
                    try:
                        db_location = models.Location(
                            **location_data.dict(), ittid=db_hotel.ittid
                        )
                        db.add(db_location)
            ...
            if hotel.provider_mappings:
                for provider_data in hotel.provider_mappings:
                    try:
                        ...
                        if existing_mapping:
                            logger.warning(
                                f"Duplicate provider mapping detected: {provider_data.provider_name}:{provider_data.provider_id}"
                            )
                            continue

                        db_provider_mapping = models.ProviderMapping(
                            **provider_data.dict(), ittid=db_hotel.ittid
                        )
                ...
                for contact_data in hotel.contacts:
                    try:
                        db_contact = models.Contact(
                            **contact_data.dict(), ittid=db_hotel.ittid
                        )
                        db.add(db_contact)
                ...
                for chain_data in hotel.chains:
                    try:
                        db_chain = models.Chain(
                            **chain_data.dict(), ittid=db_hotel.ittid
                        )
                        db.add(db_chain)
            ...
            logger.info(
                f"Successfully created hotel '{db_hotel.name}' with ITTID: {db_hotel.ittid}"
            )
...
def add_provider(
    ...
    try:
        ...
        if not ittid:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="ITTID is required"
            )
        ...
        logger.info(
            f"Adding provider mapping: {provider_name}:{provider_id} for ITTID {ittid} by user {current_user.id}"
        )
        ...
        try:
            ...
            if existing:
                logger.info(
                    f"Provider mapping already exists: {provider_name}:{provider_id} -> ITTID {existing.ittid}"
                )
                ...
                existing_dict = {
                    ...
                    "updated_at": (
                        existing.updated_at.isoformat() if existing.updated_at else None
                    ),
                }
        ...
        try:
            ...
            provider_dict = {
                ...
                "created_at": (
                    provider_mapping.created_at.isoformat()
                    if provider_mapping.created_at
                    else None
                ),
                "updated_at": (
                    provider_mapping.updated_at.isoformat()
                    if provider_mapping.updated_at
                    else None
                ),
            }
            ...
            # Log successful creation
            logger.info(
                f"Successfully created provider mapping: {provider_name}:{provider_id} -> ITTID {ittid}"
            )
            ...
            db.rollback()
            logger.error(
                f"Database integrity error creating provider mapping: {integrity_error}"
            )
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
...
def get_supplier_info(
    ...
    try:
        ...
        logger.info(
            f"Supplier info requested for '{supplier}' by user {current_user.id} (role: {current_user.role})"
        )
        ...
        try:
            if current_user.role in ["super_user", "admin_user"]:
                ...
                logger.info(
                    f"Full access granted to {current_user.role} for supplier '{supplier}'"
                )
                ...
                if not user_permission:
                    logger.warning(
                        f"Access denied for user {current_user.id} to supplier '{supplier}' - no permission found"
                    )
                    raise HTTPException(
                ...
                logger.info(
                    f"Permission-based access granted for user {current_user.id} to supplier '{supplier}'"
                )

            else:
                # Unknown or invalid role
                logger.error(
                    f"Unknown user role '{current_user.role}' for user {current_user.id}"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
        ...
        try:
            ...
            supplier_summary = (
                db.query(models.SupplierSummary)
                .filter(models.SupplierSummary.provider_name == supplier)
                .first()
            )
            ...
            if supplier_summary:
                ...
                logger.debug(
                    f"Using cached summary for '{supplier}': {total_hotel} hotels"
                )
                ...
                logger.warning(
                    f"Supplier summary not found for '{supplier}', checking if supplier exists"
                )

                # Quick existence check using LIMIT 1 (much faster than count)
                supplier_exists = (
                    db.query(models.ProviderMapping)
                    .filter(models.ProviderMapping.provider_name == supplier)
                    .limit(1)
                    .first()
                )
                ...
                last_updated = None
                logger.warning(
                    f"Supplier '{supplier}' found but summary not generated. Run count_and_save_summary.py"
                )
        ...
        logger.info(
            f"Supplier info successfully retrieved for '{supplier}' - {total_hotel} hotels found"
        )
        ...
        if supplier_summary:
            ...
            if supplier_summary.summary_generated_at:
                supplier_info_response["summary_generated_at"] = (
                    supplier_summary.summary_generated_at.isoformat()
                )
...
@router.put("/update-supplier-info", response_model=Dict[str, Any])
def update_supplier_info(
    ...
    - Recalculates total hotels & mappings
    - Updates SupplierSummary table
    - Clears cached supplier info
    ...
    if current_user.role not in ["super_user", "admin_user"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admin users can update supplier information",
        )

    if not supplier:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Supplier name is required"
        )

    supplier = supplier.strip().lower()

    logger.info(
        f"Supplier update requested for '{supplier}' by user {current_user.id} ({current_user.role})"
    )
    ...
    try:
        ...
        if not supplier_exists:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Supplier '{supplier}' not found in the system",
            )
        ...
        if not summary:
            ...
            summary.total_hotels = total_hotels
        ...
        # -------------------------------
        try:
            cache.delete(f"get-supplier-info:{supplier}")
            logger.info(f"Cache cleared for supplier '{supplier}'")
        except Exception as cache_error:
            logger.warning(f"Cache invalidation failed: {cache_error}")

        logger.info(
            f"Supplier '{supplier}' updated successfully "
            f"(hotels={total_hotels}, mappings={total_mappings})"
        )
        ...
        return {
            "status": "success",
            "message": f"Supplier '{supplier}' summary updated successfully",
            "supplier_info": {
                "supplier_name": supplier,
                "total_hotels": total_hotels,
                "total_mappings": total_mappings,
            ...
            "updated_by": {"user_id": current_user.id, "role": current_user.role},
        }
        ...
        db.rollback()
        logger.error(f"Database error updating supplier '{supplier}': {db_error}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Database error while updating supplier summary",
        )

    except Exception as e:
        db.rollback()
        logger.error(f"Unexpected error updating supplier '{supplier}': {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Unexpected error occurred while updating supplier",
        )
...
def get_user_accessible_suppliers(
    ...
    try:
        ...
        logger.info(
            f"Accessible suppliers requested by user {current_user.id} (role: {current_user.role})"
        )
        ...
        try:
            ...
            if current_user.role in ["super_user", "admin_user"]:
                    ...
                    accessible_suppliers.append(
                        {
                            ...
                            "availabilityStatus": (
                                "active" if stat.hotel_count > 0 else "inactive"
                            ),
                        }
                    )
                ...
                logger.info(
                    f"Providing permission-based supplier access to general user {current_user.id}"
                )
                ...
                total_suppliers_in_system = db.query(
                    func.count(models.SupplierSummary.id)
                ).scalar()
        ...
        active_suppliers = len(
            [s for s in accessible_suppliers if s["availabilityStatus"] == "active"]
        )
        inactive_suppliers = len(
            [s for s in accessible_suppliers if s["availabilityStatus"] == "inactive"]
        )
        access_coverage_percentage = (
            round((len(accessible_suppliers) / total_suppliers_in_system * 100), 2)
            if total_suppliers_in_system > 0
            else 0
        )

        # Log successful response
        logger.info(
            f"Successfully retrieved {len(accessible_suppliers)} accessible suppliers for user {current_user.id}"
        )
        ...
        return {
            ...
            "responseMetadata": {"generatedAt": datetime.utcnow().isoformat()},
        }
