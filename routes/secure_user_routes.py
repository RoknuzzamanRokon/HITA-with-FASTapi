...
async def create_user_secure(
    ...
    try:
        ...
        validation_result = security_validator.validate_user_creation_data(validation_data)
        ...
        existing_user = db.query(models.User).filter(
            ...
            (models.User.username == validation_result['sanitized_data']['username'])
        ).first()
...
async def update_user_secure(
    ...
    try:
        ...
        validate_user_permissions(current_user, [models.UserRole.SUPER_USER, models.UserRole.ADMIN_USER], target_user)
        
        # Prepare update data for validation
        update_dict = update_data.dict(exclude_unset=True)
        if update_dict:
            validation_result = security_validator.validate_user_creation_data(update_dict)
        ...
        if 'username' in sanitized_data:
            changes['username'] = {'old': target_user.username, 'new': sanitized_data['username']}
            target_user.username = sanitized_data['username']
        
        if 'email' in sanitized_data:
            changes['email'] = {'old': target_user.email, 'new': sanitized_data['email']}
        ...
        if update_data.role and update_data.role != target_user.role:
            changes['role'] = {'old': target_user.role.value, 'new': update_data.role.value}
...
async def delete_user_secure(
    ...
    try:
        ...
        active_transactions = db.query(models.PointTransaction).filter(
